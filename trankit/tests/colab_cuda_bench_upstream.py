# Trankit CUDA Benchmark for Colab T4 — upstream master (baseline)
# Runs the same per-task + full-pipeline benchmark for comparison
...
# ── 2. Install upstream trankit ──────────────────────────────
!pip uninstall -y trankit adapters 2>/dev/null
!pip install --no-cache-dir -q --no-deps --force-reinstall git+https://github.com/nlp-uoregon/trankit.git@master
...
# ── 3. Patch: skip TPipeline import (needs old AdamW) ────────
import importlib
import trankit as _tk_mod
# Upstream __init__.py imports TPipeline which fails with modern transformers.
# We only need Pipeline for benchmarking, so patch the import.
_init_file = _tk_mod.__file__
...
REPORT_PATH = "cuda_benchmark_upstream_report.txt"
...
# ── 5. Initialize ────────────────────────────────────────────
print(f"\n{'=' * 70}")
print(f"Trankit CUDA Benchmark — {EMBEDDING} (upstream master)")
print(f"Warmup: {WARMUP_RUNS} | Runs: {BENCHMARK_RUNS}")
print(f"{'=' * 70}\n")

torch.cuda.empty_cache()
t0 = time.perf_counter()
# Upstream Pipeline doesn't support cache_adapters
...
json_path = f"benchmark_results_{EMBEDDING.replace('/', '-')}_{gpu_name}_upstream.json"
